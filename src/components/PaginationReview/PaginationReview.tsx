...
export const PaginationReview = (props: PaginationReviewProps) => {
  ...
  const statePages = useAppSelector(
    state => state.searchResult.currentMovie.review
  );
  ...
  useEffect(() => {
    if (items !== undefined && items.length > 0) {
      const newDataPagination = items.map((item, index) => {
      ...
      setDataPagination([<Text>Нет отзывов</Text>]);
    }
    ...
    <Container m={'0 auto'} p={'0 0'} >
      ...
      >
        ...
        <Flex direction={'column'} justify={'space-between'}>
          <Flex gap={'20px'}>{dataPagination && dataPagination[0]}</Flex>
          {items && (
            <Paginations
